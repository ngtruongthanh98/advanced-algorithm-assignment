...
def generate_packing(sorted_df, log=True):
    ...
    for row in sorted_df.index:
        ...
        if log:
            print(
                f"🛳️Vessel_{vessel.vessel_idx}, 📏{vessel.size_}, ⏱️{vessel.t_arrive}~{vessel.t_process}.")
        ...
        if log:
            print(
                f"⚓Vessel_{row}, 📌{int(result.p_start)}-{int(result.p_end)}, ⌛{int(result.t_moore)}-{int(result.t_finish)}.")
            print("")
...
for i in berth_breaks_list:
    plot([0, TIME_HORIZON], [i, i], linestyle='dashed', c=(0, 0, 0), linewidth=1.5)
